...
public class Program : MonoBehaviour
{
    ...
    void initialize()
    {
        ...
        go(300, () =>
        {
            ...
            if (!File.Exists(AppUpdateLog.GAME_IMAGE_VERSION))//用于检查更新
            {
                ...
                {
                    jo.Call("doDownloadFile", "https://download.ygo2020.xyz/ygopro2/pics.zip", AppUpdateLog.MD5_IMAGE);
                    File.Copy(AppUpdateLog.GAME_DATA_VERSION, AppUpdateLog.GAME_IMAGE_VERSION, true);
                }
            }
#endif
    ...
    public void shiftToServant(Servant to)
    {
        ...
        if (to == room && room.isShowed == false) { room.show(); BGMController.Instance.StartBGM(BGMController.BGMType.siding); }
        ...
        if (to == roomList && !roomList.isShowed) { roomList.show(); BGMController.Instance.StartBGM(BGMController.BGMType.siding); }
